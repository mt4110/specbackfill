...
For Week 7 beta dogfood and the Week 8 decision, inspect the evaluator's `threshold checks`, `week 7 beta dogfood checks`, and `beta threshold checks` sections. A beta decision needs the normal continuation checks plus 60+ rows, 3+ repositories/corpora, at least one human action caused by a useful obligation, useful rate >= 25%, false-positive rate <= 20%, and error-severity false-positive rate <= 5%. Blocking consideration additionally needs 10+ useful actioned rows plus manual privacy/release gap checks.
